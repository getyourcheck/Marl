...
# Adapted from: https://gitlab.pjlab.org.cn/openmmlab/bigmodel/rl3m/-/blob/main/rl3m/layers/loss.py#L37
    ...
    def __init__(self, parallel_output=True, label_smoothing=0):
        ...
        if parallel_output:
            self.loss_fn = FlashCrossEntropyLoss(
                ...
                process_group=None,  # TODO: gpc.get_group(ParallelMode.TENSOR)
...
class PretrainLoss(FlashGPTLMLoss):
    """
    Modified from pretrain/sft loss, but with a loss factor term to balance with ppo policy loss
    """
