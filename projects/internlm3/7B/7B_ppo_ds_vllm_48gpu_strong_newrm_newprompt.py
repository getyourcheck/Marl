...
policy_minibatch=256
critic_minibatch=512
policy_learn_time=1
critic_learn_time=2
...
train_config = dict(
    ...
    policy_minibatch=policy_minibatch,
    critic_minibatch=critic_minibatch,
    policy_learn_time=policy_learn_time,
    critic_learn_time=critic_learn_time,
    ppo_loss_weight=1.0,
    ...
    critic_criterion=CriticLoss(cliprange_value=0.2, loss_type="per_token"),
)
    ...
    policy=dict(
        ...
        trainer_config=dict(
            ...
            deepspeed_config={
                ...
                'gradient_accumulation_steps': sp_size * (policy_minibatch + PRETRAIN_BATCH_SIZE) // POLICY_DP_SIZE // TRAIN_MICRO_BATCH_SIZE,
                'train_batch_size': sp_size * (policy_minibatch + PRETRAIN_BATCH_SIZE),
    ...
    critic=dict(
        ...
        trainer_config=dict(
            ...
            deepspeed_config={
                ...
                'train_micro_batch_size_per_gpu': TRAIN_MICRO_BATCH_SIZE,
                'gradient_accumulation_steps': critic_minibatch // CRITIC_DP_SIZE // TRAIN_MICRO_BATCH_SIZE,
                'train_batch_size': critic_minibatch,
            },
        ),
    ),
    reference=dict(
...
assert (policy_minibatch + PRETRAIN_BATCH_SIZE) % (TRAIN_MICRO_BATCH_SIZE *
                                                    POLICY_DP_SIZE) == 0
assert (critic_minibatch) % (TRAIN_MICRO_BATCH_SIZE * CRITIC_DP_SIZE) == 0
