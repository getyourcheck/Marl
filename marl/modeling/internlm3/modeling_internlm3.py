...
class InternLM3RotaryEmbedding(nn.Module):
    """Rotary Position Embedding for the InternLM3 model. Credits to the Reddit user /u/lucidrains."""
...
class InternLM3DynamicNTKScalingRotaryEmbedding(InternLM3RotaryEmbedding):
    """InternLM3RotaryEmbedding extended with Dynamic NTK scaling.
...
class InternLM3FlashSelfAttention2(InternLM3SelfAttention):
    """
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # For details, please see the comment in InternLM3FlashAttention2 __init__.
...
class InternLM3CrossAttention(nn.Module):
    ...
    def __init__(self, config: InternLM3Config, layer_idx: Optional[int] = None):
        ...
        self.wq = nn.Linear(self.hidden_size, self.hidden_size, bias=config.bias)
        self.wo = nn.Linear(self.hidden_size, self.hidden_size, bias=config.bias)
    ...
    def forward(
        ...
        output_attentions: bool = False,
        use_cache: bool = False,  # pylint: disable=unused-argument
        cache_position: Optional[torch.LongTensor] = None,  # pylint: disable=unused-argument
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        ...
        query_states, _ = apply_rotary_pos_emb(query_states, None, cos, sin, position_ids)
...
class InternLM3FlashCrossAttention2(InternLM3CrossAttention):
    """
    InternLM3 flash attention module. This module inherits from `InternLM3Attention` as the weights of the module stays
    untouched. The only required change would be on the forward pass where it needs to correctly call the public API of
    flash attention and deal with padding tokens in case the input contains any of them.
...
INTERNLM3_ATTENTION_CLASSES = {
    "eager": {
        "self_attention": InternLM3SelfAttention,
        "cross_attention": InternLM3CrossAttention,
    },
    "flash_attention_2": {
        "self_attention": InternLM3FlashSelfAttention2,
        "cross_attention": InternLM3FlashCrossAttention2,
    },
}


# Modified from transformers.models.llama.modeling_llama.LlamaDecoderLayer with Llama->InternLM3
class InternLM3DecoderLayer(nn.Module):
    """InternLM3 Decoder Layer. This module is a single layer of the InternLM3 model."""

    def __init__(self, config: InternLM3Config, layer_idx: int, is_cross_decoder: bool = False):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
        ...
        if is_cross_decoder:
            self.attention = INTERNLM3_ATTENTION_CLASSES[config.attn_implementation]["cross_attention"](
                config=config, layer_idx=layer_idx
            )
        else:
            self.attention = INTERNLM3_ATTENTION_CLASSES[config.attn_implementation]["self_attention"](
                config=config, layer_idx=layer_idx
            )

        self.feed_forward = InternLM3MLP(config)
        self.attention_norm = InternLM3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.ffn_norm = InternLM3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        output_hidden_states: Optional[bool] = False,  # pylint: disable=unused-argument
        ...
        if self.is_cross_decoder:
            hidden_states, self_attn_weights = self.attention(
...
class InternLM3SelfDecoder(nn.Module):
    """Self decoder layer for InternLM3 model. This module is a stack of InternLM3DecoderLayer.

    The self decoder layer inhierits the design of YOCO self decoder, which is a stack of decoder layers.
    Each decoder layer consists of a self-attention layer (GQA) and a feed-forward layer.

    Args:
        config (:class:`~transformers.InternLM3Config`): The config class to instantiate the model with.
    ...
    def __init__(self, config: InternLM3Config):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([InternLM3DecoderLayer(config, i) for i in range(config.num_self_decoder_layers)])
...
class InternLM3CrossDecoder(nn.Module):
    """Cross decoder layer for InternLM3 model. This module is a stack of InternLM3DecoderLayer.

    The cross decoder layer inhierits the design of YOCO cross-decoder, which is a stack of decoder layers.
    Each decoder layer consists of a self-attention layer (GQA) and a feed-forward layer.

    Before entering the cross decoder layer, the hidden states are projected to key and value states.
    Then the key and value states will be shared across all cross layers.
    ...
    def __init__(self, config: InternLM3Config):
        ...
        self.wk = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.bias)
        self.wv = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.bias)

        self.norm = InternLM3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)

        self.layers = nn.ModuleList(
            [InternLM3DecoderLayer(config, i, is_cross_decoder=True) for i in range(config.num_cross_decoder_layers)]
        )
    ...
    def forward(
        ...
        if past_key_values is not None:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            key_states, value_states = past_key_values.update(key_states, value_states, len(self.layers), cache_kwargs)
...
# Copied from transformers.models.llama.modeling_llama.LlamaPreTrainedModel with Llama->InternLM3
@add_start_docstrings(
...
class InternLM3PreTrainedModel(PreTrainedModel):
    """
    InternLM3 pretraiend model's base class.
    """
...
# Modified from transformers.models.llama.modeling_llama.LlamaModel with Llama->InternLM3
@add_start_docstrings(
    "The bare InternLM3 Model outputting raw hidden-states without any specific head on top.",
    InternLM3_START_DOCSTRING,
)
class InternLM3Model(InternLM3PreTrainedModel):
    """
    Transformer decoder consisting of *config.num_self_decoder_layers + config.num_cross_decoder_layers* layers.
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.tok_embeddings(input_ids.to(torch.long))
        ...
        hidden_states, all_hidden_states, all_self_attns, next_decoder_cache = self.self_decoder(
            hidden_states,
        ...
        hidden_states, all_hidden_states, all_self_attns, next_decoder_cache = self.cross_decoder(
            hidden_states,
            attention_mask=causal_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
...
class InternLM3ForCausalLM(InternLM3PreTrainedModel):
    """Causal language model (CLM) for InternLM3."""
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->InternLM3
@add_start_docstrings(
    """
    The InternLM3 Model transformer with a sequence classification head on top (linear layer).

    [`InternLM3ForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
class InternLM3ForSequenceClassification(InternLM3PreTrainedModel):
    """Sequence Classification Head for InternLM3 Model."""
...
# Copied from transformers.models.llama.modeling_llama.LlamaForQuestionAnswering with Llama->InternLM3
@add_start_docstrings(
    """
The InternLM3 Model transformer with a span classification head on top for extractive question-answering tasks like
SQuAD (a linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    InternLM3_START_DOCSTRING,
)
class InternLM3ForQuestionAnswering(InternLM3PreTrainedModel):
    """Question Answering model for InternLM3."""

    base_model_prefix = "transformer"

    def __init__(self, config):
        super().__init__(config)
        self.transformer = InternLM3Model(config)
...
# Copied from transformers.models.llama.modeling_llama.LlamaForTokenClassification with Llama->InternLM3
@add_start_docstrings(
    """
    The InternLM3 Model transformer with a token classification head on top (a linear layer on top of the hidden-states
    output) e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    InternLM3_START_DOCSTRING,
)
class InternLM3ForTokenClassification(InternLM3PreTrainedModel):
    """Token classification model for InternLM3."""
