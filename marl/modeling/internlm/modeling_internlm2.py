...
class InternLM2FlashAttention2(InternLM2Attention):
    ...
    def _upad_input(
        ...
        if query_length == kv_seq_len:
            ...
            ) = unpad_input(  # pylint: disable=E0606
...
class InternLM2ForRewardModel(InternLM2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.model = InternLM2Model(config)
        self.vocab_size = config.vocab_size
        if config.two_linear_reward_head:
    ...
    def forward(
        ...
        reward_scores = torch.gather(hidden_states.squeeze(-1), 1, ends).squeeze(-1)
    ...
    def get_score(
        ...
        **kwargs,
        ...
        if input_ids[0, -1] != self.reward_token_id:
            input_ids = torch.cat(
                [input_ids, torch.tensor([[self.reward_token_id]], dtype=torch.long)],
                dim=1,
            )
...
class InternLM2ForCriticModel(InternLM2ForRewardModel):
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        hidden_states = self.v_head(hidden_states)
        # get shifted values
        logits = hidden_states.squeeze(-1)[:, :-1]
